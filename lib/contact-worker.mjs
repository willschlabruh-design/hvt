 ...
 *
 * All secrets come from context.env passed by functions/api/contact.js.
 * Do not use process.env here — it is undefined on Cloudflare Workers.
 */
import {
  validatePayload,
  buildEmailContent,
...
export async function handleContactRequest(body, env) {
  console.info('[contact] runtime diagnostics', {
    handlerVersion: 'contact-worker-v2',
    runtime: 'cloudflare-context.env',
    vercelEnv: null,
    supportEmailDefined: Boolean(env?.SUPPORT_EMAIL),
    emailProviderDefined: Boolean(env?.EMAIL_PROVIDER),
    resendKeyDefined: Boolean(env?.RESEND_API_KEY),
    fromEmailDefined: Boolean(env?.FROM_EMAIL),
    matchingEnvKeyNames: Object.keys(env || {})
      .filter((k) => /^(SUPPORT|SMTP|RESEND|FROM_EMAIL|EMAIL|CONTACT)_/i.test(k) || k === 'EMAIL_PROVIDER' || k === 'CONTACT_DEBUG')
      .sort(),
  });
